...
const userLinks = [
  ...
  {
    to: APP_ROUTES.user.announcements,
    label: "Announcements",
    icon: Megaphone,
  },
];
...
const adminLinks: DashboardNavLink[] = [
  ...
  {
    to: APP_ROUTES.admin.withdrawals,
    label: "Withdrawal Approval",
    icon: CreditCard,
    badgeKey: "pendingWithdrawals",
  },
  {
    to: APP_ROUTES.admin.payouts,
    label: "Payouts",
    icon: HandCoins,
  ...
  {
    to: APP_ROUTES.admin.plans,
    label: "Plan Management",
    icon: FileText,
    badgeKey: "pendingPlanPurchases",
  },
  {
  ...
  {
    to: APP_ROUTES.admin.announcements,
    label: "Announcements",
    icon: Megaphone,
  },
];
...
const superAdminLinks: DashboardNavLink[] = [
  {
    to: APP_ROUTES.superAdmin.dashboard,
    label: "Dashboard",
    icon: LayoutDashboard,
    end: true,
  },
  {
    to: APP_ROUTES.superAdmin.admins,
    label: "Admin Management",
    icon: UserCog,
  },
  {
    to: APP_ROUTES.superAdmin.auditLogs,
    label: "Audit Logs",
    icon: ClipboardList,
  },
  {
    to: APP_ROUTES.superAdmin.platformSettings,
    label: "Platform Settings",
    icon: Settings,
  },
  {
    to: APP_ROUTES.superAdmin.landingPage,
    label: "Landing Page Editor",
    icon: FileText,
  },
  {
    to: APP_ROUTES.superAdmin.payoutCorrections,
    label: "Payout Corrections",
    icon: HandCoins,
  },
  {
    to: APP_ROUTES.superAdmin.transactions,
    label: "Transaction Review",
    icon: Activity,
  },
  {
    to: APP_ROUTES.superAdmin.security,
    label: "Security Control",
    icon: ShieldCheck,
  },
  {
    to: APP_ROUTES.superAdmin.roles,
    label: "Roles & Permissions",
    icon: UsersRound,
  },
  {
    to: APP_ROUTES.superAdmin.notifications,
    label: "Notifications",
    icon: Bell,
  },
  {
    to: APP_ROUTES.superAdmin.support,
...
export function DashboardLayout({ role }: DashboardLayoutProps) {
  ...
  const controlPanelSubtitle =
    role === "super_admin" ? "Governance Console" : "Management Console";
  const controlProfileRoute =
    role === "super_admin"
      ? APP_ROUTES.superAdmin.profile
      : APP_ROUTES.admin.profile;
  const controlInitial = role === "super_admin" ? "S" : "A";
  const controlNavigationBase =
    role === "super_admin" ? superAdminLinks : adminLinks;
  const adminNavigationLinks = controlNavigationBase.map((item) => ({
    ...item,
    badge: item.badgeKey
      ? String(adminPendingCounts[item.badgeKey])
      : undefined,
  }));
  ...
  useEffect(() => {
    const isMenuOpen = isControlPanel
            ...
            <aside className="absolute right-0 top-0 flex h-[100dvh] w-[min(88vw,360px)] flex-col border-l border-cyan-300/15 bg-[#020918] text-white shadow-[-28px_0_80px_rgba(2,8,23,0.42)]">
                ...
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="truncate text-sm font-bold leading-none tracking-wide">
                      {controlPanelLabel} Menu
                    </p>
                    <p className="mt-1 text-[11px] font-medium text-cyan-100/65">
                      Tap a module to open
                    </p>
                  </div>
                </div>
                <button
              ...
              <ScrollArea className="min-h-0 flex-1">
                <nav className="grid gap-2 px-4 py-4">
                            ...
                            {item.badge ? (
                              <span
                                className={cn(
                                  ...
                                  isActive
                                    ? "border-slate-950/20 text-slate-950/80"
                  ...
                  <button
                    className="group flex min-h-11 items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-semibold text-slate-300 transition-all duration-200 active:bg-cyan-300/[0.08] active:text-cyan-100 disabled:pointer-events-none disabled:opacity-70"
                  ...
                  {logoutMutation.error ? (
                    <p className="px-3 text-[11px] font-medium text-rose-200">
                      {logoutMutation.error}
                    </p>
                  ) : null}
                </nav>
        ...
        <aside className="sticky top-0 z-40 hidden flex-col border-b border-cyan-300/15 bg-[#020918] text-white shadow-[0_20px_70px_rgba(2,8,23,0.28)] lg:flex lg:h-screen lg:border-b-0 lg:border-r lg:border-cyan-300/15">
          <div
            className={cn(
              ...
              isAdminSidebarCollapsed
                ? "lg:flex-col lg:gap-2 lg:px-3 lg:py-3"
                : "justify-between",
            )}
          >
            <div
              className={cn(
                ...
                isAdminSidebarCollapsed &&
                  "lg:flex-col lg:justify-center lg:gap-1",
              )}
            >
              <span className="grid size-10 shrink-0 place-items-center rounded-2xl border border-cyan-300/35 bg-cyan-300/10 shadow-[0_0_24px_rgba(34,211,238,0.24)]">
                <img
          ...
          <ScrollArea className="min-h-0 flex-1 lg:h-[calc(100vh-15.5rem)]">
            <nav className="flex gap-2 px-3 py-3 sm:grid sm:grid-cols-2 lg:grid-cols-1 lg:py-4 lg:pr-4">
                      ...
                      <>
                        <Icon
                          className={cn(
                            ...
                            isActive
                              ? "text-slate-950"
                              : "text-cyan-100/78 group-hover:text-cyan-100",
                          )}
                        />
                        <span
                          className={cn(
              ...
              <button
                className={cn(
                  "group flex shrink-0 items-center gap-3 rounded-xl px-3 py-2.5 text-left text-xs font-semibold text-slate-300 transition-all duration-200 hover:bg-cyan-300/[0.08] hover:text-cyan-100 sm:shrink lg:w-full",
                  isAdminSidebarCollapsed && "lg:justify-center lg:px-0",
                ...
                type="button"
              >
                <LogOut className="size-4 shrink-0 text-cyan-100/78 group-hover:text-cyan-100" />
                <span
                  className={cn(
                    "min-w-0 flex-1 truncate",
                    isAdminSidebarCollapsed && "lg:hidden",
              ...
              {logoutMutation.error ? (
                <p
                  className={cn(
                    "px-3 text-[11px] font-medium text-rose-200",
                    isAdminSidebarCollapsed && "lg:hidden",
                  )}
                >
                  {logoutMutation.error}
          ...
          <div
            className={cn(
              "border-t border-cyan-300/10 p-3",
              isAdminSidebarCollapsed && "lg:px-2",
            )}
          >
            <NavLink
              className={({ isActive }) =>
            ...
            >
              ...
              <ShieldCheck
                className={cn(
                  "ml-auto size-4 text-emerald-300",
                  isAdminSidebarCollapsed && "lg:hidden",
                )}
              />
            </NavLink>
      ...
      <header className="sticky top-0 z-40 flex items-center justify-between border-b border-cyan-300/20 bg-gradient-to-r from-[#020918] via-[#061225] to-[#07324a] px-4 py-3 text-white shadow-[0_18px_46px_rgba(2,8,23,0.34)] lg:hidden">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-[11px] font-semibold text-cyan-100/80 truncate">
              {userDisplayName}
            </p>
          </div>
        </div>
                      ...
                      {({ isActive }) => (
                        <>
                          <Icon
                            className={cn(
                              "size-4 shrink-0",
                              isActive ? "text-slate-950" : "text-cyan-100/78",
                            )}
                          />
                          <span className="min-w-0 flex-1 truncate">
                            {item.label}
      ...
      <aside className="sticky top-0 z-40 hidden h-screen flex-col border-r border-cyan-300/15 bg-[#020918] text-white shadow-[0_20px_70px_rgba(2,8,23,0.28)] lg:flex">
        ...
        >
          ...
          >
            ...
            <div
              className={cn("min-w-0", isUserSidebarCollapsed && "lg:hidden")}
            >
              <p className="truncate text-sm font-bold leading-none tracking-wide">
                ARBITRUM
              </p>
              <p className="mt-1 text-[11px] font-medium text-cyan-100/65 truncate">
                {userDisplayName}
              </p>
            </div>
          </div>
        ...
        <ScrollArea className="min-h-0 flex-1 lg:h-[calc(100vh-13rem)]">
          <nav
            className={cn(
              "grid gap-2 px-3 py-4 pr-4",
              isUserSidebarCollapsed && "lg:px-2",
            )}
          >
            {userLinks.map((item) => {
              const Icon = item.icon;
                  ...
                  {({ isActive }) => (
                    <>
                      ...
                      <span
                        className={cn(
                          "min-w-0 flex-1 truncate",
                          isUserSidebarCollapsed && "lg:hidden",
                        )}
                      >
                        {item.label}
                      </span>
