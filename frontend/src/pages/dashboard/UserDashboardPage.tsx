...
function buildChartPoints(
  ...
  return source.map((point, index) => ({
    ...point,
    x: source.length === 1 ? 168 : 48 + (index * 252) / (source.length - 1),
  ...
  return (
    <section className="space-y-3 sm:space-y-4">
      ...
      <div className="grid gap-3 sm:gap-4 xl:grid-cols-[1.15fr_0.85fr]">
        <Card className="form-motion-off overflow-hidden rounded-2xl border border-cyan-500/15 bg-[#0b1628] text-white shadow-[0_20px_60px_rgba(6,182,212,0.08)]">
          <CardHeader className="p-4 pb-2">
            <div className="flex items-start justify-between gap-3">
              ...
              <span className="rounded-full border border-cyan-500/20 bg-cyan-950/50 px-3 py-1 text-[11px] font-bold text-cyan-400">
                This Year
              </span>
            </div>
            <div className="mt-3 grid gap-2 sm:grid-cols-3">
              <div className="rounded-2xl border border-slate-700/30 bg-slate-800/40 px-3 py-2">
                <p className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Start</p>
                <p className="mt-1 text-sm font-black text-white">{formatUsdt(chartStats.first)}</p>
              </div>
              <div className="rounded-2xl border border-slate-700/30 bg-slate-800/40 px-3 py-2">
                <p className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Peak</p>
                <p className="mt-1 text-sm font-black text-cyan-400">{formatUsdt(chartStats.peak)}</p>
              </div>
              <div className="rounded-2xl border border-slate-700/30 bg-slate-800/40 px-3 py-2">
                <p className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Growth</p>
                <p className={`mt-1 text-sm font-black ${chartStats.growth >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
                  {chartStats.growth >= 0 ? "+" : ""}
          ...
          <CardContent className="p-4 pt-0">
            <div className="relative h-64 overflow-hidden rounded-2xl border border-slate-700/40 bg-gradient-to-b from-[#0d1b2e] to-[#091422] sm:h-72">
              <svg
              ...
              >
                <defs>
                  <linearGradient id="uBarGradient" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#22d3ee" />
                    <stop offset="50%" stopColor="#3b82f6" />
                    <stop offset="100%" stopColor="#1d4ed8" />
                  </linearGradient>
                  <linearGradient id="uCurveGradient" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#22d3ee" stopOpacity="0.08" />
                    <stop offset="100%" stopColor="#22d3ee" stopOpacity="0" />
                  </linearGradient>
                  <linearGradient id="uArrowLineGradient" x1="0" y1="1" x2="1" y2="0">
                    <stop offset="0%" stopColor="#34d399" />
                    <stop offset="100%" stopColor="#22d3ee" />
                  </linearGradient>
                  <filter id="uArrowGlow" height="200%" width="200%" x="-50%" y="-50%">
                  ...
                  <filter id="uBarGlow" x="-30%" y="-10%" width="160%" height="120%">
                ...
                {[30, 48, 66, 84, 102, 120, 138].map((lineY) => {
                  const dataRange = chartStats.peak || 100;
                  const rawVal = ((132 - lineY) / 76) * dataRange;
                ...
                <path
                  ...
                  fill="url(#uCurveGradient)"
                />

                {/* Premium Capsule Bars with Neon Glow */}
                  ...
                  return (
                    ...
                    >
                      <rect
                        ...
                        height={barH}
                        rx="8"
                        fill="url(#uBarGradient)"
                        stroke="#22d3ee"
                        strokeWidth="0.5"
                        strokeOpacity="0.25"
                        className="transition-all duration-300 hover:brightness-125"
                      />
                      {/* Inner highlight capsule */}
                      <rect
                        x={cx - 4}
                        y={cy + 2}
                        width={4}
                        height={Math.max(barH - 8, 2)}
                        rx="2"
                        fill="white"
                        fillOpacity="0.08"
                      />
                ...
                <path
                  ...
                  fill="none"
                  stroke="url(#uArrowLineGradient)"
                  strokeWidth="3"
                  strokeLinecap="round"
                  filter="url(#uArrowGlow)"
                />
              </svg>
              <div className="absolute inset-0 pointer-events-none">
                {/* Neon Arrowhead aligned with tangent */}
                {lastPoint && (
                  <div
                    className="absolute"
                    style={{
                ...
                {hoveredPoint && (
                  ...
                  >
                    <p className="text-[9px] font-black text-cyan-400 uppercase tracking-wider">{hoveredPoint.month} Overview</p>
          ...
          <div className="flex items-center justify-around border-t border-slate-700/40 px-4 py-3 text-[11px] font-bold text-slate-400">
            {chartPoints.length >= 2 && (() => {
              ...
              const isUp = curr.amountUsdt >= prev.amountUsdt;
              return (
                <>
                  <div className="flex items-center gap-1.5">
                    <span className="inline-block size-2 rounded-full bg-cyan-400 shadow-[0_0_6px_#22d3ee]" />
                    <span>{prev.month}: <strong className="text-white">{formatChartUsdt(prev.amountUsdt)}</strong></span>
                    <span className="text-emerald-400 text-[10px]">▲</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className="inline-block size-2 rounded-full bg-emerald-400 shadow-[0_0_6px_#34d399]" />
                    <span>{curr.month}: <strong className="text-white">{formatChartUsdt(curr.amountUsdt)}</strong></span>
