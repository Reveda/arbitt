...
import { useEmailVerificationRequest, useVerifyEmail } from "@/hooks/useAuthActions";
...
export function VerifyEmailPage() {
  ...
  const handleResendOtp = async () => {
    ...
    if (!EMAIL_PATTERN.test(email)) {
      setError("email", { message: "Enter a valid email address.", type: "manual" });
      return;
    }

    try {
      await resendOtpMutation.mutate({ email });
    } catch {
      // Error state is handled by the shared API mutation hook.
    }
  };
  ...
  return (
    <Card className="form-motion-off mx-auto w-full max-w-xl">
      <CardHeader>
        ...
        <CardDescription>Enter the 6-digit OTP sent for your account.</CardDescription>
      </CardHeader>
      <CardContent>
        <form className="grid gap-4" noValidate onSubmit={onSubmit}>
          ...
          <label className="grid gap-2 text-sm font-medium">
            ...
            <Input
              ...
              className={errors.otp ? "border-red-400/70 focus-visible:ring-red-300" : ""}
              inputMode="numeric"
              maxLength={6}
              placeholder="000000"
              type="text"
            />
            {errors.otp ? <span className="text-xs font-normal text-red-300">{errors.otp.message}</span> : null}
          </label>
          ...
          <Button className="w-full" disabled={verifyEmailMutation.isLoading} type="submit">
